...
function calcularDiaSemana($dia,$mes,$ano) {
 ...
 function gerarCalendario($mes,$ano,$nmeses,$ncols,$datas,$rodapes,$leg,$dia_ceia,$semana_ceia)//$feriados,$marcados,$rodapes)
 {
  ...
  {
   $tabela="Erro ao gerar calend�rio: [m&ecirc;s=".$mes."] [ano=".$ano.
           "] [n�mero de meses=".$nmeses."] [tabelas por linha=".$ncols."]<br>";
  } else {
   //Carrega o css do calend�rio e armazena em css/calendario.css
   ...
   $dia=$daux=$dia_semana;
    for($i=0;$i<count($datas);$i++)
      $qtd[$i]=count($datas[$i]);
     $nq=count($qtd);
     $tabela="<table class='table'>"; //Inicia a tabela geral (que suportar� as demais tabelas de meses)
     while($idx<$total)
     {
      $tabela .= "<tr>";
      for($ms=0; $ms<$ncols && $idx<$total; $ms++)
      {
       $temp_tb="<td valign='top'><table class='table table-bordered'>
                <tr class='info'><td colspan='7'><h4>".$meses[$idx]."/".$ano.
                "</h4></td></tr><tr>"; //Cria uma tabela para o m�s atual
   ...
   if ($ceia=="" &&  $d==$dia_ceia && $semana==$semana_ceia) //Se 1� Sexta marca santa ceia
	...
	$temp_ln=$temp_ln.'<td class="'.$classe.'" '.$titulo.' >  '.$cnt_dias++.'</td>';
     ...
     //Gera um rodap� para a tabela de m�s
   ...
   $i=1;//define a cor para idenficar a legenda no calend�rio
    $legenda=$legenda."<tr><td class='td_marcado8'>&nbsp;</td><td class='td_leg'>".$leg."</td></tr>";
