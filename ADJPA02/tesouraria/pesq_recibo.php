...
<form id="form" name="form" method="get" action="">
  <input name="nome" type="text" autofocus="autofocus" id="nome" size="15" tabindex="1" /> 
...
if (!empty($_GET['nome']))
	{
		$query  = "SELECT m.rol, m.nome,t.recebeu FROM membro AS m,tes_recibo AS t ";
		$query .= "WHERE m.nome LIKE '%".trim($_GET["nome"])."%' AND m.rol=t.recebeu";
		$query .= " AND t.tipo=1 GROUP BY m.rol ORDER BY m.nome ";
	...
	$_urlLi="pesq_recibo.php?nome=".$_GET["nome"]."&campo=".$_GET["campo"]."&rol=".$_GET["rol"]."&form=".$_GET["form"];//Montando o Link para ser passada a classe
