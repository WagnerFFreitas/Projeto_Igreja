...
class cuserlevels extends cTable {
	...
	function __construct() {
		...
		// userlevelid
		$this->userlevelid = new cField('userlevels', 'userlevels', 'x_userlevelid', 'userlevelid', '`userlevelid`', '`userlevelid`', 3, -1, FALSE, '`userlevelid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->userlevelid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['userlevelid'] = &$this->userlevelid;

		// userlevelname
		$this->userlevelname = new cField('userlevels', 'userlevels', 'x_userlevelname', 'userlevelname', '`userlevelname`', '`userlevelname`', 200, -1, FALSE, '`userlevelname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`userlevelid` ASC";
	}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('userlevelid', $rs))
				ew_AddFilter($where, ew_QuotedName('userlevelid') . '=' . ew_QuotedValue($rs['userlevelid'], $this->userlevelid->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("userlevelsview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("userlevelsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "userlevelsadd.php?" . $this->UrlParm($parm);
		else
			return "userlevelsadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("userlevelsedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("userlevelsadd.php", $this->UrlParm($parm));
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["userlevelid"]; // userlevelid
   ...
   // Common render codes
		...
		// userlevelname
		$this->userlevelname->CellCssStyle = "width: 250px;";
