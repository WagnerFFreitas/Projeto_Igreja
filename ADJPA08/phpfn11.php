...
// Classe de carregamento automático
function ew_AutoLoad($class) {
	global $EW_RELATIVE_PATH;
	if (substr($class, 0, 1) == "c") {
		if ($class == "csistemigreja_db") {
			$file = "sistemigrejadb.php";
		} else {
		$fn = "%cls%info.php";
		$file = str_replace("%cls%", substr($class, 1), $fn);
		$file = strtolower($file);
		}
		if (file_exists($EW_RELATIVE_PATH . $file))
			include $EW_RELATIVE_PATH . $file;
	}
}
...
// Get current project ID - (ID do projeto atual)
function CurrentProjectID() {
	if (isset($GLOBALS["Page"]))
		return $GLOBALS["Page"]->ProjectID;
	return "{2B7992FC-5911-46A7-9310-01F4D4225C49}";
}
...
// Obtém o objeto da tabela do usuário
function &UserTable() {
	return $GLOBALS["UserTable"];
}

// Obtém o objeto da tabela principal atual
function &CurrentTable() {
	return $GLOBALS["Table"];
}

// Obter o objeto da tabela principal atual
...
class cNumericPager {
	...
	// Setup pager items (Configurar itens do pager)
...
// Common class for table and report (Classe comum para mesa e relatório)
class cTableBase {
	var $TableVar;
	var $TableName;
	var $TableType;
	...
	// Setup field titles (Configurar títulos de campo)
...
class cField {
	...
	function ViewAttributes() {
		$sAtt = "";
		$sStyle = "";
		if ($this->FldViewTag == "IMAGE" && intval($this->ImageWidth) > 0 && (!$this->ImageResize || ($this->ImageResize && intval($this->ImageHeight) <= 0)))
			$sStyle .= "width: " . intval($this->ImageWidth) . "px; ";
		if ($this->FldViewTag == "IMAGE" && intval($this->ImageHeight) > 0 && (!$this->ImageResize || ($this->ImageResize && intval($this->ImageWidth) <= 0)))
...
class cListOptions {
	...
	// Add and return a new option (Adicione e devolva uma nova opção)
	function &Add($Name) {
	...
	// Load default settings (Carregar configurações padrão)
	...
	// Hide all options (Ocultar todas as opções)
	...
	// Show all options (Mostrar todas as opções)
...
// Obtém o item por nome
// Nomes predefinidos: visualizar / editar / copiar / apagar / detalhe_ <DetailTable> / userpermission / checkbox
	...
	// Move item to position (Mover item para posição)
	function MoveItem($Name, $Pos) {
		$cnt = count($this->Items);
		if ($Pos < 0) // Se negativo, conte a partir do final
			$Pos = $cnt + $Pos;
		if ($Pos < 0)
			$Pos = 0;
		if ($Pos >= $cnt)
	...
	// Render list options (Opções de lista de renderização)
	...
	function RenderDropDownButton($body, $pos) {
		...
		if ($links <> "") {
			...
			$button = '<button class="' . $buttonclass .'"' . $buttontitle . ' data-toggle="dropdown">' . $this->DropDownButtonPhrase . '<b class="caret"></b></button><ul class="dropdown-menu ewMenu">' . $links . '</ul>';
	...
	function ValidateUser(&$usr, &$pwd, $autologin) {
		...
		if (!$ValidateUser) {
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					$ValidateUser = $CustomValidateUser || ew_ComparePassword($rs->fields('senha'), $pwd);
					if ($ValidateUser) {
						$_SESSION[EW_SESSION_STATUS] = "login";
						$_SESSION[EW_SESSION_SYS_ADMIN] = 0; // Non System Administrator
						$this->setCurrentUserName($rs->fields('login')); // Load user name
						$this->setSessionUserID($rs->fields('u')); // Load User ID
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if ($RsUser = $conn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('u'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
...
function Database_Connected(&$conn) {
	$sSql = "Select * from smtp";
	$rs1 = $conn->Execute($sSql);
	$_SESSION['se'] = $rs1->fields[1];
	$_SESSION['po'] = $rs1->fields[2];
	$_SESSION['us'] = $rs1->fields[3];
	$_SESSION['pw'] = $rs1->fields[4];
	$_SESSION['em'] = $rs1->fields[5];
	$_SESSION['sc'] = $rs1->fields[7];
}
...
function ew_ExecuteJson($SQL, $FirstOnly = TRUE) {
...
function ew_ResizeBinary(&$filedata, &$width, &$height, $quality) {
	...
	if (ew_ResizeThumb($thumb, $width, $height, $quality)) {
		$filedata = $thumb->getImageAsString();
		return TRUE;
	}
	return FALSE;
}
...
function ew_ResizeFile($fn, $tn, &$width, &$height, $quality) {
	...
	$thumb = PhpThumbFactory::create($fn);
	if (ew_ResizeThumb($thumb, $width, $height, $quality)) {
		$thumb->save($tn);
	} else {
		if ($fn <> $tn) copy($fn, $tn);
...
// Resize PhpThumb
function ew_ResizeThumb(&$thumb, &$width, &$height, $quality) {
	$dimensions = $thumb->getCurrentDimensions();
	if ($width <= 0) { // maintain aspect ratio
		$h = $height;
		$w = $dimensions["width"] * $height / $dimensions["height"];
	} elseif ($height <= 0) { // maintain aspect ratio
		$w = $width;
		$h = $dimensions["height"] * $width / $dimensions["width"];
	} else {
		$w = $width;
		$h = $height;
	}
	if (EW_REDUCE_IMAGE_ONLY && $w >= $dimensions["width"] && $h >= $dimensions["height"]) {
		return FALSE;
	} else {
		$options = $thumb->getOptions();
		$options["keepAspectRatio"] = EW_KEEP_ASPECT_RATIO;
...
// Check token
function ew_CheckToken($token) {
	$mlt = intval(ini_get("session.gc_maxlifetime"));
	if ($mlt <= 0)
		$mlt = 1440; // PHP default
	return (time() - intval(ew_Decrypt($token))) < $mlt;
}
