...
$query=mysql_query("SELECT p.numero,p.local,DATE_FORMAT(p.dt_liberado,'%d/%m/%Y �s %H:%i') dt_lib,p.chefe, p.quant,p.obs, p.quant_atendida,p.usu_libera,pr.codbarras,pr.descricao,pr.id FROM pedido AS p, produtos AS pr WHERE p.numero='$numero' AND p.id_produto=pr.id");
?>
...
<?PHP
//fim do 1� if que verifica a autoriza��o para NUPAT ou NUTEL
}else{
